  ...
  useEffect(() => {
    const loadSurveys = async () => {
      ...
      if (result.ok === 1 && result.item) {
        // 최신순 정렬
        const sorted = result.item.sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
        setSurveys(sorted);
      }
      setIsLoading(false);
    };
    ...
    return (
      <TabCard>
        ...
        <div className="text-center py-10">
          ...
          <p className="text-yg-darkgray mt-4">설문 기록을 불러오는 중...</p>
        </div>
      </TabCard>
    );
  }
    ...
    return (
      <TabCard>
        ...
        <div className="text-center py-10">
          ...
          <Button onClick={() => (window.location.href = '/survey')} variant="primary" size="lg">
            설문 시작하기
          </Button>
        </div>
      </TabCard>
  ...
  return (
    <TabCard>
      ...
      <div className="space-y-4">
        {surveys.map((survey) => (
          <div key={survey._id} className="border border-yg-primary rounded-[30px] p-6 hover:shadow-lg transition cursor-pointer" onClick={() => onSurveyClick(survey.target_id)}>
            <div className="flex justify-between items-center">
              <div>
                ...
                <p className="text-yg-darkgray text-xs mt-1">추천 영양제 {survey.memo.supplements.length}개</p>
              </div>
              <button className="bg-yg-primary rounded-full w-10 h-10 flex items-center justify-center shadow-lg">
