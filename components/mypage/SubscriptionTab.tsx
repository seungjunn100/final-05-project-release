  ...
  const handleToggleStatus = async (subscriptionId: string, currentStatus: 'active' | 'paused') => {
    ...
    if (result.ok === 1) {
      setSubscriptionInfo((prev) => ({
        ...prev,
        products: prev.products.map((p) => (p._id === subscriptionId ? { ...p, status: newStatus } : p)),
      }));
      alert(`구독이 ${newStatus === 'active' ? '재개' : '일시정지'}되었습니다.`);
    } else {
    ...
    return (
      <TabCard>
        ...
        <div className="text-center py-10">
          ...
          <p className="text-yg-darkgray mt-4">구독 정보를 불러오는 중...</p>
        </div>
      </TabCard>
    );
  }
    ...
    <TabCard>
      <div className="flex items-center justify-between mb-6">
        ...
        {subscriptionInfo.isSubscribed && (
          <button onClick={handleClearAll} disabled={isClearing} className="text-sm text-yg-darkgray hover:text-red-500 transition disabled:opacity-50 cursor-pointer">
      ...
      {subscriptionInfo.isSubscribed ? (
        <div className="space-y-4">
          {subscriptionInfo.products.map((product) => (
            <div key={product._id} className="border border-yg-primary rounded-[30px] p-6">
              {/* 상품 정보 */}
              <div className="flex items-center gap-4 mb-4">
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className={`px-3 py-1 rounded-full text-xs font-semibold ${product.status === 'active' ? 'bg-yg-primary text-white' : 'bg-yg-gray text-white'}`}>{product.status === 'active' ? '활성' : '일시정지'}</span>
                  </div>
                  <p className="text-yg-darkgray text-sm">월 {product.price.toLocaleString()}원</p>
                </div>
              </div>
              ...
              <div className="bg-yg-white rounded-[20px] p-4 mb-4">
                ...
                {product.status === 'active' && (
                  <div className="flex justify-between text-sm">
                    ...
                    <span className="font-semibold text-yg-primary">{product.nextPaymentDate}</span>
                  </div>
                )}
              </div>

              {/* 상태 변경 버튼 */}
              <Button onClick={() => handleToggleStatus(product._id, product.status)} disabled={updatingProductId === product._id} variant={product.status === 'active' ? 'secondary' : 'primary'} className="w-full">
                {updatingProductId === product._id ? '처리 중...' : product.status === 'active' ? '구독 일시정지' : '구독 재개'}
              </Button>
            </div>
          ))}
        </div>
        ...
        <div className="text-center py-10">
          ...
          <Button onClick={onNavigateToSubscription} variant="primary" size="lg">
            구독하러 가기
          </Button>
        </div>
      )}
